...
public final class ShaderGroup {
    ...
    public void initAll() {
        if (this.getFragmentShader() != null) {
            Game.getGame().getLogManager().log("Initializing " + this.getFragmentShader().getShaderName() + this.getFragmentShader().getShaderType().getFile());
            this.getFragmentShader().init();
            this.getUniformsFullSet().addAll(this.getFragmentShader().getUniforms());
        }
        if (this.getVertexShader() != null) {
            Game.getGame().getLogManager().log("Initializing " + this.getVertexShader().getShaderName() + this.getVertexShader().getShaderType().getFile());
            this.getVertexShader().init();
            this.getUniformsFullSet().addAll(this.getVertexShader().getUniforms());
        }
        if (this.getGeometricShader() != null) {
            Game.getGame().getLogManager().log("Initializing " + this.getGeometricShader().getShaderName() + this.getGeometricShader().getShaderType().getFile());
