...
public class RenderEntity implements IRenderFabric {
    ...
    public void onRender(double partialTicks, SceneRenderBase sceneRenderBase, IRenderItem iRenderItem) {
        ...
        sceneRenderBase.performUniform("model_view_matrix", sceneRenderBase.getSceneWorld().getRenderManager().getTransform().getModelViewMatrix(entityItem.getEntityModel().getMeshModel(), sceneRenderBase.getSceneWorld().getRenderManager().getTransform().getViewMatrix(sceneRenderBase.getSceneWorld().getCamera())));
        EntityModel.EntityForm entityForm = entityItem.getEntityModel().getPropForm();
        GL30.glBindVertexArray(entityItem.getEntityModel().getMeshModel().getModel3D().getVao());
